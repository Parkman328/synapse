...
# Rename this function to be generic for UIA auth password login too
...
class AuthHandler(BaseHandler):
    ...
    async def _check_auth_dict(
        ...
        submission = client_dict_convert_legacy_fields_to_identifier(authdict)

        # Extract a user ID from the values in the identifier
        username = await self.username_from_identifier(
            submission["identifier"],
        )
    ...
    async def username_from_identifier(self, identifier: Dict[str, str]) -> Optional[str]:
        ...
        if identifier["type"] == "m.id.thirdparty":
            ...
            canonical_user_id, _ = await self.check_password_provider_3pid(
                medium, address, identifier["password"]  # TODO: Wait, we don't have a password...
            )
